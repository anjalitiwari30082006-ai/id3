...
def entropy(col):
    ...
    return sum([(-counts[i]/len(col)) * math.log2(counts[i]/len(col)) for i in range(len(values))])
...
if data_option == "Synthetic Tennis Data":
    data_dict = {
        "Outlook": ["sunny", "sunny", "overcast", "rain", "rain", "overcast", "sunny", "sunny", "overcast", "rain", "overcast"],
        "Humidity": ["high", "normal", "high", "normal", "high", "high", "normal", "normal", "normal", "normal", "normal"],
...
if st.button("Generate Tree & Analyze Features"):
    ...
    gain_df = pd.DataFrame(list(gains.items()), columns=["Feature", "Information Gain"]).sort_values(by="Information Gain")
